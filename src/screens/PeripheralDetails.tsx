...
const PeripheralDetailsScreen = ({route}: PeripheralDetailsProps) => {
  ...
  // Function to write a string to a characteristic
  const writeData = async (
    ...
    // Convert the message from UTF-8 to Base64
  ...
  const readData = async (
    ...
    try {
      const base64Value = await BleManager.read(
        peripheralId,
        serviceUUID,
        characteristicUUID,
      );
      const message = Buffer.from(base64Value, 'base64').toString('utf8');
